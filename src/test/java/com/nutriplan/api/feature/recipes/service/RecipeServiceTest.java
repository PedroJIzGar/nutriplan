...
class RecipeServiceTest {
        ...
        @Test
        @DisplayName("getAllRecipes should return mapped recipe summary list")
        void getAllRecipesShouldReturnMappedRecipeSummaryList() {
                ...
                when(recipeMapper.toSummaryResponseList(List.of(recipe))).thenReturn(List.of(response));

                List<RecipeSummaryResponse> result = recipeService.getAllRecipes();

                assertThat(result).containsExactly(response);

                verify(recipeRepository).findAllByActiveTrueOrderByCreatedAtDesc();
                verify(recipeMapper).toSummaryResponseList(List.of(recipe));
        }
        ...
        private ArgumentMatcher<Recipe> matchesRecipeBuiltFromRequest() {
                return recipe -> recipe.getName().equals("Chicken Rice Bowl")
                                ...
                                && recipe.getIngredients().stream().anyMatch(ri -> "grilled".equals(ri.getNotes()))
                                && recipe.getIngredients().stream().anyMatch(ri -> ri.getNotes() == null);
        }
}